...
namespace httpHelper
{
    ...
    public class HttpWebConnect
    {
     ...
     , ex.Message));
                #if LOG_OUTPUT
    txtLog.LogError("HttpWebConnect -> TryPostData"
					,ex);
#endif
        ...
        private void RequestStreamCallBack(IAsyncResult asynchronousResult)
        {
            try
            {
                ...
                RequestState _myRequestState = (RequestState)asynchronousResult.AsyncState;
                HttpWebRequest myrequest = _myRequestState.request;
                // End the operation.
                ...
                postStream.Close();
                myrequest.BeginGetResponse(new AsyncCallback(RespCallback), _myRequestState);

            }
            catch (System.Exception ex)
            {
                #if LOG_OUTPUT
    txtLog.LogError("HttpWebConnect -> RequestStreamCallBack"
					,ex);
#endif
                this.RaiseException(ex);
            }
        ...
        public void TryRequest()
        {
            ...
            {
                #if LOG_OUTPUT
    txtLog.LogError("HttpWebConnect -> TryRequest"
					,e);
#endif
                Debug.WriteLine(
    ...
    , e.Message));
        ...
        void RaiseEvents(RequestState rs)
        {
            if (this.RequestCompleted != null && rs != null)
            {

                //Deployment.Current.Dispatcher.BeginInvoke(() => RequestCompleted(this.myRequestState.requestData.ToString()));
                this.RequestCompleted(rs.requestData.ToString());
            }
        }
        void RaiseEvents()
        {
        ...
        private void RespCallback(IAsyncResult asynchronousResult)
        {
            ...
            try
            {
                ...
                RequestState _myRequestState = (RequestState)asynchronousResult.AsyncState;
                HttpWebRequest myHttpWebRequest = _myRequestState.request;
                _myRequestState.response = (HttpWebResponse)myHttpWebRequest.EndGetResponse(asynchronousResult);

                // Read the response into a Stream object.
                Stream responseStream = _myRequestState.response.GetResponseStream();
                _myRequestState.streamResponse = responseStream;

                // Begin the Reading of the contents of the HTML page and print it to the console.
                IAsyncResult asynchronousInputRead = responseStream.BeginRead(_myRequestState.BufferRead, 0, BUFFER_SIZE, new AsyncCallback(ReadCallBack), _myRequestState);
                //return;
            }
            #region 异常处理
            ...
            {
                #if LOG_OUTPUT
    txtLog.LogError("HttpWebConnect -> RespCallback"
					,e);
#endif
                this.RaiseException(e);
                //if (this.AcceptException != null)
        ...
        private void ReadCallBack(IAsyncResult asyncResult)
        {
            ...
            {
                #if LOG_OUTPUT
    txtLog.LogError("HttpWebConnect -> ReadCallBack"
					,e);
#endif
                this.RaiseException(e);
                //if (this.AcceptException != null)
